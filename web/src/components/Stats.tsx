...
function formatBps(bps: number) {
  ...
  const kb = bps / 1024
  if (kb < 1024) return `${kb.toFixed(1)} KiB/s`
  const mb = kb / 1024
  if (mb < 1024) return `${mb.toFixed(2)} MiB/s`
  const gb = mb / 1024
...
function StatsPanel({ role }: { role: 'inbound' | 'outbound' }) {
  const isInbound = role === 'inbound'
  // 历史曲线窗口
  const [win, setWin] = React.useState<number>(5 * 60 * 1000)
  ...
  React.useEffect(() => {
    let mounted = true
    const snap = isInbound ? API.inbound.statsSnapshot() : API.outbound.statsSnapshot()
    snap.then(s => {
      if (!mounted) return
      const tunnels = (s.tunnels || []).map((t: any) => ({ id: t.id, tag: t.tag, entry_port: t.entry_port, up: 0, down: 0, bytesUp: t.uplink, bytesDown: t.downlink }))
      setWsLast({ ts: s.ts, bytes: { up: s.total?.uplink || 0, down: s.total?.downlink || 0 }, tunnels })
    }).catch(()=>{})
  ...
  React.useEffect(() => {
    ...
    const range = isInbound ? API.inbound.statsRange(since, selId || undefined) : API.outbound.statsRange(since, selId || undefined)
    range.then(r => {
      if (!mounted) return
      const s = (r.series || []).map((p: any) => ({ t: p.ts, up: p.uplink, down: p.downlink }))
      setSeries(s)
  ...
  React.useEffect(() => {
    ...
    ws.onmessage = (ev) => {
      try {
        ...
        const roleMatch = isInbound ? raw.role === 'portal' : raw.role === 'bridge'
        if (!roleMatch) return
        const norm = {
          ts: raw.ts as number,
        ...
        if (selId) {
          const it = (norm.tunnels as any[] | undefined)?.find(t => t.id === selId)
    ...
    <div className="grid gap-4">
      <Card>
        <div className="flex items-center justify-between">
          <CardTitle>{isInbound ? '入口速率趋势' : '出口速率趋势'}</CardTitle>
          <div className="flex gap-2 items-center">
            <select className="rounded-md border border-slate-300 bg-white px-2 py-1 text-sm dark:border-slate-700 dark:bg-slate-900" value={selId} onChange={e=>setSelId(e.target.value)}>
              <option key="all" value="">{isInbound ? '全部入口' : '全部隧道'}</option>
              {(((wsLast?.tunnels as any[])||[])).map((t: any, idx: number) => (
                <option key={t.id || t.entry_port || t.tag || idx} value={t.id}>{t.tag || t.id}（{t.entry_port}）</option>
              ))}
            </select>
        ...
        <div className="mt-2">{series.length > 1 ? <LineChart points={series} /> : <div className="text-sm text-slate-500">暂无数据</div>}</div>
      </Card>
      <Card>
        <CardTitle>{isInbound ? '各入口当前速率' : '各隧道当前速率'}</CardTitle>
          ...
          {(() => {
            ...
            return tunnels.map((t: any, idx: number) => (
              <div key={t.id || t.entry_port || t.tag || idx} className="rounded-lg border border-slate-200 p-3 dark:border-slate-800">
                <div className="flex items-center justify-between text-sm"><div className="font-medium">{t.tag || t.id}</div><div className="text-xs text-slate-500">{isInbound ? '入口' : '端口'} {t.entry_port}</div></div>
...
export default function Stats() {
  return (
    <div className="grid gap-6">
      <StatsPanel role="inbound" />
      <StatsPanel role="outbound" />
    </div>
  )
}
