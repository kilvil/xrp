...
export default function Stats() {
  ...
  React.useEffect(() => {
    let mounted = true
    API.statsRange(Date.now()-win, entryId || undefined).then(r => {
      if (!mounted) return
  ...
  React.useEffect(() => {
    ...
    ws.onmessage = (ev) => {
      try {
        ...
        if (entryId) {
          const it = (norm.tunnels as any[] | undefined)?.find(t => t.id === entryId)
      ...
      <Card>
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2 items-center">
            <select className="rounded-md border border-slate-300 bg-white px-2 py-1 text-sm dark:border-slate-700 dark:bg-slate-900" value={entryId} onChange={e=>setEntryId(e.target.value)}>
              <option key="all" value="">全部入口</option>
